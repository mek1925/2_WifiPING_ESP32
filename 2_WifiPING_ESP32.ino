...
static int c = 0, d = 0, x = 0, s = 0, p = 0;
int print = 3;
...
void shiftWriteEEPROMdata(int startOffset, int *endOffset, int lastAddr, String readString)
{
    int lastOffset = *endOffset;
    int newLastOffset;
    char data[EEPROM_SIZE];
    // String data;
    char check[EEPROM_SIZE];
    int len = readString.length() + 1;
    uint32_t diff;

    Serial.println("Befor EEPROM data:-");
    fullReadEEPROM();
    Serial.println("");
    ...
    if (len < lastOffset)
    {
        ...
        // newLastOffset = writeStringToEEPROM(startOffset, readString);
        // *endOffset = newLastOffset;
        diff = lastOffset - newLastOffset;
        while (diff != 0) //&& a >= newLastOffset)
        {

            // left Shift EEPROM data
            for (int i = 0; i < EEPROM_SIZE; i++)
            {
                data[i] = EEPROM.read(a + i);
                EEPROM.write(a - 1 + i, data[i]);
                check[i] = EEPROM.read(a - 1 + i);
        ...
        Serial.println("EEPROM left shift done.");
        Serial.println("After EEPROM data:-");
        fullReadEEPROM();
        Serial.println("");
    }
    if (len > lastOffset)
    {
        int a = lastAddr;
        diff = len - lastOffset;
        // right Shift EEPROM data
        while (diff != 0)
        {
            for (int i = a; i >= lastOffset; i--)
            {
                data[i] = EEPROM.read(i);
                EEPROM.write(1 + i, data[i]);
                check[i] = EEPROM.read(1 + i);
                // Serial.print(check);
                // EEPROM.commit();
            }
            Serial.print(check);
        ...
        Serial.println("EEPROM Right shift done.");
        Serial.println("After EEPROM data:-");
        fullReadEEPROM();
        Serial.println("");
        // newLastOffset = writeStringToEEPROM(startOffset, readString);
